// hatsu_ino v1 — 3D-printable enclosure
//
...
// Nano, DFPlayer Mini, and speaker with calipers and adjust the values
// in this section before printing — module dimensions vary between
// suppliers, and the DFPlayer Mini module especially varies in exact
// footprint and microSD slot position between clones.
//
// Render/export:
...
pcb_t     = 1.6;   // standard PCB thickness (Nano / DFPlayer Mini)
floor_t   = 2.0;   // base floor thickness
lid_t     = 2.0;   // lid plate thickness

// ── Text labels ────────────────────────────────────────────────────
lid_label_text  = "hatsu_ino v1";
...
// Floor plan: the speaker occupies its own zone (X 0-58), fully separate
// from the electronics zone (X 58-105) where the Nano and DFPlayer Mini
// both sit flush against the far wall, stacked along Y.
in_l = 105; // interior length (X)
in_w = 60;  // interior width  (Y)
in_h = 28;  // interior height (Z), measured from floor top face — sized for a 50mm/3W speaker

// ── Arduino Nano ───────────────────────────────────────────────────
// The Mini-USB connector is on the board's SHORT edge — the Nano sits
// flush against the far (X = in_l) wall so that edge, and the USB
// cutout below, actually line up.
nano_l = 45;
nano_w = 18;
...
// ── DFPlayer Mini ──────────────────────────────────────────────────
// Assumed square-ish footprint with the microSD slot on a short edge —
// VERIFY this against your actual module once it arrives; DFPlayer Mini
// clones vary in exact size and slot position more than most breakouts.
dfp_l = 20;
dfp_w = 20;
dfp_sd_slot_w = 15;  // microSD width + finger clearance
dfp_sd_slot_h = 3.0;
dfp_pos_x = in_l - dfp_l;   // flush against the far wall, same side as the USB cutout
dfp_pos_y = 29;

// ── Speaker (assume ~50mm, 4Ω, 3W — VERIFY against your actual part) ─
// Centered in its own zone (X 0-58), clear of the electronics zone.
spk_d       = 50 + 1.0;  // + fit clearance
...
module base() {
    difference() {
        ...
        // microSD card slot — cut through the far wall, aligned with the DFPlayer Mini
        translate([wall + in_l - 1, wall + dfp_pos_y + (dfp_w - dfp_sd_slot_w) / 2,
                   floor_t + pcb_t + 1])
            cube([wall + 2, dfp_sd_slot_w, dfp_sd_slot_h + tol]);
    ...
    translate([wall, wall, 0]) {
        ...
        pcb_edge_rails(dfp_l, dfp_w, dfp_pos_x, dfp_pos_y);
...
module lid() {
    ...
    translate([wall + (58 + in_l) / 2, wall + in_w / 2, lid_t])
